...
ZERO_DATASETS = [
    ...
    (
        ...
        "entities",
        {
            "seen": {"Location", "Price", "Hours"},
            "unseen": {"Rating", "Amenity", "RestaurantName", "Dish", "Cuisine"},
        },
        {
            "TP": 0,
            "total_pos": 0,
...
for name, path in PATHS.items():
    ...
    for seed_no, seed in enumerate(SEEDS):
        ...
        for elems in ZERO_DATASETS_iter:
            try:
                dataset, task, entity_dict, seen_scores, unseen_scores, general_f1 = (
                    elems
                )
            except ValueError as e:
                raise ValueError(f"{elems}\n{e}")
            for class_name, class_score in results[dataset][task][
                "class_scores"
            ].items():
                if class_name in entity_dict["seen"]:
                    ...
                    seen_entity_f1[f"{dataset}_{class_name}"][seed_no][
                        "TP"
                    ] += class_score["tp"]
                    seen_entity_f1[f"{dataset}_{class_name}"][seed_no][
                        "total_pos"
                    ] += class_score["total_pos"]
                    seen_entity_f1[f"{dataset}_{class_name}"][seed_no][
                        "total_pre"
                    ] += class_score["total_pre"]
                    ...
                    unseen_entity_f1[f"{dataset}_{class_name}"][seed_no][
                        "TP"
                    ] += class_score["tp"]
                    unseen_entity_f1[f"{dataset}_{class_name}"][seed_no][
                        "total_pos"
        ...
        for _, _, _, seen_scores, unseen_scores, _ in ZERO_DATASETS_iter:
            precision = (
                seen_scores["TP"] / seen_scores["total_pre"]
                if seen_scores["total_pre"] > 0
                else 0.0
            )
            recall = (
                seen_scores["TP"] / seen_scores["total_pos"]
                if seen_scores["total_pos"] > 0
                else 0.0
            )
            seen_scores["F1"] = (
            ...
            precision = (
                unseen_scores["TP"] / unseen_scores["total_pre"]
                if unseen_scores["total_pre"] > 0
                else 0.0
            )
            recall = (
                unseen_scores["TP"] / unseen_scores["total_pos"]
                if unseen_scores["total_pos"] > 0
                else 0.0
            )
            unseen_scores["F1"] = (
                2 * precision * recall / (precision + recall)
    ...
    for dataser_no, dataset_name in enumerate(dataset_names):
        ...
        for it in range(len(iter_results)):
            dataset, task, entity_dict, seen_scores, unseen_scores, f1 = iter_results[
                it
            ][dataser_no]
    ...
    for entity_name, scores in unseen_entity_f1.items():
        for i in range(len(scores)):
            precision = (
                scores[i]["TP"] / scores[i]["total_pre"]
                if scores[i]["total_pre"] > 0
                else 0.0
            )
            recall = (
                scores[i]["TP"] / scores[i]["total_pos"]
                if scores[i]["total_pos"] > 0
                else 0.0
            )
            scores[i]["F1"] = (
                2 * precision * recall / (precision + recall)
    ...
    for entity_name, scores in unseen_entity_f1.items():
        unseen_entity_f1_avg[entity_name] = np.array(
            [score["F1"] for score in scores]
        ).mean(0)

    for entity_name, scores in seen_entity_f1.items():
        seen_entity_f1_avg[entity_name] = np.array(
            [score["F1"] for score in scores]
        ).mean(0)
