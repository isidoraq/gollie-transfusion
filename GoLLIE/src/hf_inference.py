...
def main(
    ...
    if use_lora:
        ...
        if not os.path.exists(model_path):
            ...
            print(
                f"##### Merging lora from {lora_path} to GoLLIE-7B and save to {model_path}"
            )
            merge_lora_model(
                weights_path="HiTZ/GoLLIE-7B",
    ...
    for task_name in task_name_list.split(","):
        ...
        for data_file in task_list:
            save_file_name = os.path.join(
                predictions_folder, f"{data_file}.predictions.jsonl"
            )
            ...
            prompts = [l["text"].split("result =")[0] + "result =" for l in data][
                :num_size
            ]
    ...
    for task_name in task_name_list.split(","):
        print("Task Name: ", task_name)
        task_scores = load_json(
            os.path.join(output_path, f"{task_name}_task_scores_summary.json")
        )
        if task_name == "redfm":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_lora", action="store_true", help="Whether to use LoRA or not"
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default=None,
        help="Path to save the generated outputs (optional)",
    )
    parser.add_argument(
        "--lora_path",
        type=str,
        default=None,
        help="Path to the LoRA weights file (required if --use_lora is True)",
    )
    parser.add_argument(
        "--model_name",
        type=str,
        default=None,
        help="Path to model name from huggingface",
    )
