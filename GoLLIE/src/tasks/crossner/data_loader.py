...
class CrossNERDatasetLoader(DatasetLoader):
    ...
    def __init__(
        self, path_or_split: str, tasks: str, include_misc: bool = True, **kwargs
    ) -> None:
        if len(tasks) > 1:
            raise ValueError(
        ...
        task_definitions, task_target = {
            "CrossNER_POLITICS": (
                (
                    ENTITY_DEFINITIONS_POLITICS
                    if include_misc
                    else ENTITY_DEFINITIONS_POLITICS_woMISC
                ),
                "entities",
            ),
            "CrossNER_AI": (
                ENTITY_DEFINITIONS_AI if include_misc else ENTITY_DEFINITIONS_AI_woMISC,
                "entities",
            ),
            "CrossNER_NATURAL_SCIENCE": (
                (
                    ENTITY_DEFINITIONS_NATURAL_SCIENCE
                    if include_misc
                    else ENTITY_DEFINITIONS_NATURAL_SCIENCE_woMISC
                ),
                "entities",
            ),
            "CrossNER_LITERATURE": (
                (
                    ENTITY_DEFINITIONS_LITERATURE
                    if include_misc
                    else ENTITY_DEFINITIONS_LITERATURE_woMISC
                ),
                "entities",
            ),
            "CrossNER_MUSIC": (
                (
                    ENTITY_DEFINITIONS_MUSIC
                    if include_misc
                    else ENTITY_DEFINITIONS_MUSIC_woMISC
                ),
                "entities",
            ),
        }[task]
