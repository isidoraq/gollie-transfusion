...
class WikiEventsPreprocessor:
    ...
    def _sent_tokenize(
        self: WikiEventsPreprocessor, text: str, start_pos: int
    ...
    def _find_subsentence(offset, sentences):
        sentences_ = sentences.copy() + [
            (sentences[-1][0] + len(sentences[-1][1]) + 1, "")
        ]
        return next(
            (i - 1 for i, (idx, _) in enumerate(sentences_) if offset < idx), -1
        )
    ...
    def __call__(self, input_path: str) -> Iterable[dict]:
        ...
        with open(input_path, "rt") as data_f:
            for data_line in tqdm(data_f):
                ...
                all_sub_sentences = [
                    list(self._sent_tokenize(text, tokens[0][1]))
                ...
                for value in entities.values():
                    ...
                    sub_sent_idx = self._find_subsentence(
                        tokens[value["start"]][1], all_sub_sentences[sent_idx]
                    )

                    sentence = sub_sentence_information[f"{sent_idx}-{sub_sent_idx}"][
                        "text"
                    ]
                    ...
                    new_value["start"] = (
                        ...
                        - all_sub_sentences[sent_idx][sub_sent_idx][0]
                    )
                    # Fix start if needed
                    ...
                    assert (
                        sentence[new_value["start"] : new_value["end"]]
                        == new_value["text"]
                    ), f"{sentence[new_value['start']:new_value['end']]}|{new_value['text']}"

                    if (
                        "entity_mentions"
                        not in sub_sentence_information[f"{sent_idx}-{sub_sent_idx}"]
                    ):
                        sub_sentence_information[f"{sent_idx}-{sub_sent_idx}"][
                            "entity_mentions"
                        ] = []
                ...
                for event in instance["event_mentions"]:
                    ...
                    # Fix start if needed
                    _shift = sentence[new_value["trigger"]["start"] :].find(
                    ...
                    # )
                    new_value["trigger"]["end"] = new_value["trigger"]["start"] + len(
                        new_value["trigger"]["text"]
                    )
                    ...
                    sent_entities = (
                        {
                            ent["id"]
                            for ent in sub_sentence_information[
                                f"{sent_idx}-{sub_sent_idx}"
                            ]["entity_mentions"]
                        }
                        if "entity_mentions"
                        in sub_sentence_information[f"{sent_idx}-{sub_sent_idx}"]
                        else {}
                    )
                    ...
                    assert (
                        sentence[
                            new_value["trigger"]["start"] : new_value["trigger"]["end"]
                        ]
                        == new_value["trigger"]["text"]
                ...
                for i, sentences in enumerate(all_sub_sentences):
                    for j, _ in enumerate(sentences):
                        ...
                        if (
                            "entity_mentions" not in info
                            and "event_mentions" not in info
                            and len(info["text"]) <= 25
                        ):
                            continue
    ...
    for filename in os.listdir(args.dir_path):
        ...
        if os.path.isfile(full_name) and "sentence" not in filename:
            output_file = os.path.join(
                args.dir_path, filename.replace(".jsonl", ".sentence.jsonl")
            )
