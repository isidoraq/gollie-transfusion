...
def encode_data(path):
    output = []
    with open(
        "/coc/pskynet6/ychen3411/transfusion/GoLLIE/dataset/RAMS_1.0c/data/train.jsonlines",
        "rt",
    ) as in_f:
        for line in in_f:
            ...
            for event in line["evt_triggers"]:
                ...
                span_mentions.append(
                    {"start": trigger_start, "end": trigger_end + 1, "type": event_type}
                )
                ...
                if not is_overlap:
                    encoded_sentence, marker2label = easyproject_encoding_eae(
                        tokens, span_mentions
                    )
                    output.append(
                        {"sentence": encoded_sentence, "marker2label": marker2label}
                    )
    return output
...
def decode_data(input_sentence, output_sentence, marker2label):
    ...
    if is_valid:
        # Get entities
        clean_sentence, entities = prepare_data(
            output_sentence, output_entities, marker2label
        )
        return clean_sentence, entities
    else:
        return None, None
