...
def encode_data(path):
    output = []
    with open(
        "/srv/nlprx-lab/share6/dminh6/X-Gear/processed_data/ace05_en_mT5/train.json",
        "rt",
    ) as in_f:
        for line in in_f:
            ...
            if len(tokens) <= 15:
                continue
            entities = [
                {"start": entity["start"], "end": entity["end"], "type": entity["id"]}
            ...
            if not is_overlap:
                encoded_sentence, marker2label = easyproject_encoding_ace(
